  ...
  if (n == 0) {
    ...
    if (!hide[j.toLowerCase()]) {
      const raf = (y, s) => {
        ...
        ctx.font = `600 ${window.innerHeight / 25}px Metropolis`;
...
const drawBullet = (n, x, y, a) => {
  ...
  switch (n) {
    ...
    case 1:
      ...
      break;
    default:
      ctx.font = `500 ${window.innerHeight / 25}px Metropolis`;
      ctx.fillStyle = "#F55";
      ctx.textAlign = "left";
...
const cntRender = () => {
  ...
  try {
    if (comboAlert) {
      ...
      fontSize = (window.innerHeight / 100) * (30 - (comboAlertMs + 900 - Date.now()) / 90);
      ctx.beginPath();
      ...
      if (renderTriggers[i].value == 0) {
        ...
        if (renderTriggers[i].ms - 1 <= seek * 1000 && renderTriggers[i].ms + renderTriggers[i].time > seek * 1000) {
          ...
          if (renderTriggers[i].size.indexOf("vh") != -1)
            ctx.font = `${renderTriggers[i].weight} ${(window.innerHeight / 100) * Number(renderTriggers[i].size.split("vh")[0])}px Metropolis, Pretendard Variable`;
...
const calculateResult = () => {
  ...
  if (missPoint.length == 0) {
    missCtx.fillStyle = "#FFF";
    missCtx.font = `500 ${window.innerHeight / 30}px Metropolis`;
